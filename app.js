const FINAL_LINE = "CYBER KNIGHTS DEFEND";
const QR_FRAGMENTS = [
...
const state = {
  levelClears: {1:0,2:0,3:0},
  qrEarned: {1:false,2:false,3:false},
  current: 0,
};
const levelMeta = {
...
function goHome(){
  state.current = 0;
  ['home','level1','level2','level3'].forEach(id=>setVisible(id,false));
...
function renderHomeQR() {
  ...
  // ✅ Auto-fill after completing any 3 levels
  const completedLevels = Object.values(state.qrEarned).filter(Boolean).length;
  ...
  if (completedLevels >= 3) {
    if (finalInput) {
      ...
      finalInput.disabled = true; // prevent editing
    }
